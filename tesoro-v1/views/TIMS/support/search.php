<?php
	...
	if(isset($_POST['search_submit'])){
	 ...
	 $keywords = explode(" .-/:",$search);
		$query="SELECT a.id AS 'id', a.job_no AS 'jon',a.date_received,a.description AS 'des',a.due_date,a.agent,a.customer,a.artist,a.date_added,a.current_note,b.kind_of_job AS 'koj',c.job_type AS 'jt',d.status_name AS 'sn' FROM jobbings a LEFT JOIN jobbings_kinds b ON a.job_kind=b.id INNER JOIN jobbings_type c ON b.type=c.id LEFT JOIN jobbings_statuses d ON a.current_status=d.id WHERE";
		$keyCount = 0;
		foreach($keywords as $keys){
			if($keyCount > 0){
				$query .= " AND";
	...
	<div class="container-fluid">
		<div id="content">
			<div class="row">
				<div class="col-md-12">
				<?php if(
					(isset($_GET['view']) || !isset($_GET['edit']) && !isset($_GET['update']) && !isset($_GET['more']) && !isset($_GET['remove'])) && 
					(!isset($_GET['view']) || !isset($_GET['edit']) && !isset($_GET['update']) && isset($_GET['more']) && isset($_GET['remove'])) && 
					(!isset($_GET['view']) || !isset($_GET['edit']) && isset($_GET['update']) && isset($_GET['more']) && !isset($_GET['remove'])) && 
					(!isset($_GET['view']) || !isset($_GET['edit']) && isset($_GET['update']) && !isset($_GET['more']) && !isset($_GET['remove'])) &&
					(!isset($_GET['view']) || isset($_GET['edit']) && !isset($_GET['update']) && !isset($_GET['more']) && !isset($_GET['remove']))
					...
					<ul id="tabsJustified" class="nav nav-tabs">
						...
						<li class="nav-item">
							<a href="search.php" class="nav-link small text-uppercase active">
								Search
							</a>
						</li>
	    			</ul>
												...
												<tr> 
													...
													<td>
														<td>
															...
															<button class="btn btn-secondary" onclick="update('<?php echo $row['id']; ?>')" title="Update Status" style="font-size: 12px; margin-top: 2px;" disabled><img src="images/update.png" width="15px" style="margin: -3px -4px 0px -4px;">
															</button> 
			...
			?>
				<ul id="tabsJustified" class="nav nav-tabs">
					<li class="nav-item"><a href="all_jobs.php" class="nav-link small text-uppercase">All</a></li>
