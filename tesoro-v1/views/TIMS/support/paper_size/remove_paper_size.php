<?php
 if(is_numeric($_GET['remove']) && $_GET['remove']!=0){
	$query0="SELECT * FROM printing_paper_size WHERE id=".$_GET['remove'];

	$result_set0=mysqli_query($conn,$query0);
	if(mysqli_num_rows($result_set0)>0){
	 $row0=mysqli_fetch_array($result_set0);
	 $remove_query = "DELETE FROM printing_paper_size WHERE id=".$_GET['remove'];
		...
		<?php
			...
			<script type="text/javascript">
				swal({
					title: "Failed?",
					text: "Paper size is not removed!",
					type: "error",
					confirmButtonClass: "btn-primary",
					confirmButtonText: "OK",
					closeOnConfirm: false,
	 ...
	 <div style="max-height: 75vh; overflow-x: hidden; padding: 20px 10px">
		Sorry, paper size does not exist! <a class="btn btn-primary" href="<?php echo $_SESSION['page'];?>">Back</a>
	 </div>
	<?php
	} 
 }else {?>
