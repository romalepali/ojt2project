 ...
 if(is_numeric($_GET['update']) && $_GET['update']!=0){
	...
	if(mysqli_num_rows($result_set0)>0){
	 ...
	 <?php if($row0['type']==1){
          ...
          //Small Jobs Only
          ?>
        	Sorry, this job is under small job category. <a class="btn btn-primary" href="small_jobs.php?update=<?php echo $_GET['update'];?>">View Job</a>
      	<?php
        }
        else if($row0['type']==3){
          //Small Jobs Only
          ?>
        	Sorry, this job is under big job - small category. <a class="btn btn-primary" href="big_jobs_small.php?update=<?php echo $_GET['update'];?>">View Job</a>
      	<?php
        }?>
	 </div>
	 <?php
