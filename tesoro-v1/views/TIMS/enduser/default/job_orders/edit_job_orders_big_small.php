 ...
 if(is_numeric($_GET['edit']) && $_GET['edit']!=0){
	...
	if(mysqli_num_rows($result_set0)>0){
	 ...
	 <?php if($row0['type']==1){
	 	//Big Jobs Only
		?>
        Sorry, this job is under big job category. <a class="btn btn-primary" href="big_jobs_small.php?edit=<?php echo $_GET['edit'];?>">Edit Job</a>
      <?php
	 }
	 else if($row0['type']==2){
	 	//Small Jobs Only
		?>
        Sorry, this job is under small job category. <a class="btn btn-primary" href="small_jobs.php?edit=<?php echo $_GET['edit'];?>">Edit Job</a>
      <?php
	 }
	 else if($row0['type']==3){
	 	include ('edit/edit_big_small.php');
	 }?>
	 </div>
	 <?php
	} else {?>
