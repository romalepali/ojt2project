<?php
	if(isset($_GET['add']) && $_GET['add']=='small'){?>
		...
		<div style="max-height: 75vh; overflow-x: hidden; padding: 20px 10px">
	 		Sorry, you can't add a small job on a big job! <a class="btn btn-primary" href="big_jobs.php?add=big">Add Big Jobbing</a>
		</div><?php
	}else if(isset($_GET['add']) && $_GET['add']=='big_small'){?>
		<div style="max-height: 75vh; overflow-x: hidden; padding: 20px 10px">
	 		Sorry, you can't add a small job on a big job - small! <a class="btn btn-primary" href="big_jobs_small.php?add=big_small">Add Big Jobbing</a>
		</div><?php
	}else {?>
		<div style="max-height: 75vh; overflow-x: hidden; padding: 20px 10px">
