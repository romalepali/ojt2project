<?php
  if(is_numeric($_GET['view']) && $_GET['view']!=0){
    ...
    if(mysqli_num_rows($result_set0)>0){
      ...
      //Big Jobs Only
      ?>
        Sorry, this job is under big job category. <a class="btn btn-primary" href="big_jobs.php?view=<?php echo $_GET['view'];?>">View Job</a>
      <?php
      }
      else if($row0['type']==2){
      //Small Jobs Only
      ...
      //Big Jobs Only
      ?>
        Sorry, this job is under big job - small category. <a class="btn btn-primary" href="big_jobs_small.php?view=<?php echo $_GET['view'];?>">View Job</a>
      <?php
      }else {?>
      <div style="max-height: 75vh; overflow-x: hidden; padding: 20px 10px">
        Sorry, job does not exist! <a class="btn btn-primary" href="<?php echo $_SESSION['page'];?>">
        Back</a>
      </div>
      <?php
      } ?>
