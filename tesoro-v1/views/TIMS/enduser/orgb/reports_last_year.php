<?php
	...
	$_SESSION["filter"] = "reports_last_year";
			...
			<div class="row">
				<div class="col-md-12">
					...
					<ul id="tabsJustified" class="nav nav-tabs">
						...
						<li class="nav-item"><a href="reports_last_year.php" class="nav-link small text-uppercase active">Last <?php echo $year-1;?></a></li>
					</ul>
								...
								<table class="table table-hover" id="myTable">
									...
									if(isset($_POST['submit'])){
									 ...
									 $sql_query="SELECT a.current_status,a.job_no AS 'jon',a.description AS 'des',a.date_received,a.due_date,a.agent,a.customer,a.artist,a.pages,a.current_note,a.initial_copies, a.payment, b.kind_of_job AS 'koj',c.job_type AS 'jt',d.status_name AS 'sn',d.id FROM jobbings a LEFT JOIN jobbings_kinds b ON a.job_kind=b.id LEFT JOIN jobbings_type c ON b.type=c.id LEFT JOIN jobbings_statuses d ON a.current_status=d.id WHERE ";
											...
											if (empty($_POST['search'])&&empty($_POST['artist'])&&empty($_POST['Status'])&&empty($_POST['jobKind'])&&empty($_POST['jobType'])&&empty($_POST['agent'])&&empty($_POST['payment'])&&empty($_POST['monthFrom'])&&empty($_POST['monthTo'])) {

												$sql_query="SELECT a.current_status,a.job_no AS 'jon',a.description AS 'des',a.date_received,a.due_date,a.agent,a.customer,a.artist,a.pages,a.current_note,a.initial_copies,b.kind_of_job AS 'koj',c.job_type AS 'jt',d.status_name AS 'sn' FROM jobbings a INNER JOIN jobbings_kinds b ON a.job_kind=b.id INNER JOIN jobbings_type c ON b.type=c.id LEFT JOIN jobbings_statuses d ON a.date_received=d.id WHERE YEAR(a.date_received) = ('$year'-1) && b.type != 2 ORDER BY a.date_received DESC";
												$andCount = 2;
										...
										if($andCount!=2){
											if($andCount==1){
											 ...
											 $sql_query .= " YEAR(a.date_received) = ('$year'-1) &&  b.type != 2  ORDER BY a.date_received DESC  ";
										}
										}
