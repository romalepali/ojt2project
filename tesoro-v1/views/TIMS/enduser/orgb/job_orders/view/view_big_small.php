<?php
	...
	$query="SELECT a.job_no AS 'jon',a.description AS 'des',g.size,a.date_received,a.due_date,a.agent,a.customer,a.artist,a.date_added,a.current_note,a.cover,a.pages,a.initial_copies,f.payment,b.kind_of_job AS 'koj',c.job_type AS 'jt',c.id AS 'type',d.status_name AS 'sn',e.unit FROM jobbings a LEFT JOIN jobbings_kinds b ON a.job_kind=b.id RIGHT JOIN jobbings_type c ON b.type=c.id LEFT JOIN jobbings_statuses d ON a.current_status=d.id LEFT JOIN copies_units e ON a.copies_unit=e.id LEFT JOIN jobbings_payment f ON a.payment=f.id LEFT JOIN printing_paper_size g ON a.size=g.id WHERE a.id=".$_GET['view'];
