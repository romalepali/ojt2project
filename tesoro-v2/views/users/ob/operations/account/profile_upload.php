...
if(isset($_POST['reset'])){
	$default = 'default';
	$reset_query="SELECT picture FROM users_list WHERE id=".$_SESSION['user_id'];
	$reset_result=mysqli_query($conn,$reset_query);
	if(mysqli_num_rows($reset_result)>0){
		$del_pic=mysqli_fetch_array($reset_result);
		$pic = $del_pic[0];
		if($pic != "default"){
			$pic_dir = "../uploads/images/";
			unlink($pic_dir.$pic.".png");
		}
	}
	$sql="UPDATE users_list SET picture='$default' WHERE id=".$_SESSION['user_id'];
...
if(isset($_FILES['file'])){
	...
	if($imageFileType != "png") {
		...
		if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
			$del_query="SELECT picture FROM users_list WHERE id=".$_SESSION['user_id'];
			...
			if(mysqli_num_rows($del_result)>0){
				...
				$sql="UPDATE users_list SET picture='$removeExt' WHERE id=".$_SESSION['user_id'];
				if(mysqli_query($conn,$sql)){
				?>
				<script type="text/javascript">
					window.location.href='<?php echo $_SESSION['page'];?>';
