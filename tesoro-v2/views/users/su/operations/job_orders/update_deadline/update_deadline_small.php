<?php
	if(isset($_POST['update_deadline_small_save'])){
		$ub = $_SESSION['user_id'];
		$deadline = $_POST['deadline'];
		$notif = "";
		
		if(isset($_POST['deadline']) && $_POST['deadline']!=NULL){
			$small_save = "UPDATE jo SET deadline_on='$deadline' WHERE job_no=".$_GET['update_deadline'];
		}else{
			$small_save = "UPDATE jo SET deadline_on=NULL WHERE job_no=".$_GET['update_deadline'];
		}
...
<form action="<?php echo $_SESSION['page'];?>?update_deadline=<?php echo $_GET['update_deadline'];?>" method="POST">
	...
	<div style="float: right; padding: 20px 0px">
		...
		<button class="btn btn-primary" type="submit" name="update_deadline_small_save">Save Changes</button>
	</div>
</form>