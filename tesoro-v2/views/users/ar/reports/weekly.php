					...
					if(isset($_POST['view'])){
						...
						if(empty($_POST['dateStart'])&&empty($_POST['dateEnd'])){
							...
							if(empty($_POST['dateEnd'])){
								...
								$sql_query="SELECT a.received_on,a.job_kind,a.agent,a.artist,a.job_no as JobNo ,a.customer,a.description,a.pages,a.deadline_on,a.encoded_on,b.firstname,b.lastname,b.type,c.copies FROM jo a LEFT JOIN users_list b ON a.agent=b.id LEFT JOIN jo_copies c ON a.job_no=c.job_no WHERE a.artist=".$_SESSION['user_id']." AND a.received_on ='$d2' ORDER BY a.received_on DESC";
							}else{
								$sql_query="SELECT a.received_on,a.job_kind,a.agent,a.artist,a.job_no as JobNo ,a.customer,a.description,a.pages,a.deadline_on,a.encoded_on,b.firstname,b.lastname,b.type,c.copies FROM jo a LEFT JOIN users_list b ON a.agent=b.id LEFT JOIN jo_copies c ON a.job_no=c.job_no WHERE a.artist=".$_SESSION['user_id']." AND a.received_on BETWEEN DATE_ADD('$d1', INTERVAL 1-DAYOFWEEK('$d1') DAY) AND DATE_ADD('$d2', INTERVAL 7-DAYOFWEEK('$d2') DAY) ORDER BY a.received_on DESC";
							}
						}
					}else{
						$sql_query="SELECT a.received_on,a.job_kind,a.agent,a.artist,a.job_no as JobNo ,a.customer,a.description,a.pages,a.deadline_on,a.encoded_on,b.firstname,b.lastname,b.type,c.copies FROM jo a LEFT JOIN users_list b ON a.agent=b.id LEFT JOIN jo_copies c ON a.job_no=c.job_no WHERE a.artist=".$_SESSION['user_id']." AND a.received_on BETWEEN DATE_ADD(CURDATE(), INTERVAL 1-DAYOFWEEK(CURDATE()) DAY) AND DATE_ADD(CURDATE(), INTERVAL 7-DAYOFWEEK(CURDATE()) DAY) ORDER BY a.received_on DESC";
