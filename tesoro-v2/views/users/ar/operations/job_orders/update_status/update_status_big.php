<?php
	if(isset($_POST['update_stat_big_save'])){
		$ub = $_SESSION['user_id'];
		$job_no = $_GET['update_stat'];
		$notes = mysqli_real_escape_string($conn,$_POST['notes']);
		$status = $_POST['status'];
		
		$big_save = "INSERT INTO jo_status (job_no,status,notes,updated_on,updated_by) VALUES ('$job_no','$status','$notes',NOW(),'$ub')";
...
<form action="<?php echo $_SESSION['page'];?>?update_stat=<?php echo $_GET['update_stat'];?>" method="POST">
	...
	<div style="float: right; padding: 20px 0px">
		...
		<button class="btn btn-primary" type="submit" name="update_stat_big_save">Save Changes</button>
	</div>
</form>