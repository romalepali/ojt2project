<?php
	if(is_numeric($_GET['upload_s']) && $_GET['upload_s']!=0){
		$test_query="SELECT * FROM jo_status WHERE id=".$_GET['upload_s'];

		$test_result=mysqli_query($conn,$test_query);
		if(mysqli_num_rows($test_result)>0){
	 		$test=mysqli_fetch_array($test_result);?>
			<title>J.O. No.: <?php echo $test['job_no'];?> | Upload Sample</title>
	 		<div class="row">
				<div class="col-10">
	 		...
	 		<div style="padding: 0px 10px"><?php
				include ('upload_s/upload_sf.php');?>
			</div><?php
		} else {?>
