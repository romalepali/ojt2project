...
<title>Job Orders | Inactive</title>
	...
	<div id="all_jobs" class="tab-pane fade active show">
		<div class="table-responsive">
			<table class="table table-hover" id="myTable">
				...
				<tbody><?php
					$sql_query="SELECT a.job_no,a.description,a.customer,a.pages,a.artist,a.cover,a.deadline_on,a.encoded_on,b.job_kind,c.firstname AS 'arf',c.lastname AS 'arl',d.firstname AS 'crf',d.lastname AS 'crl' FROM jo a LEFT JOIN jo_kinds b ON a.job_kind=b.id LEFT JOIN users_list c ON a.artist=c.id LEFT JOIN users_list d ON a.cover=d.id WHERE a.artist=".$_SESSION['user_id']." ORDER BY a.encoded_on DESC";
						...
						while($jo=mysqli_fetch_assoc($result_set)){?>  
							...
							if(mysqli_num_rows($check_result)>0){
								while($check=mysqli_fetch_assoc($check_result)){
									if($check['status']>=9 && $check['status']<=10){
										if($jo['artist']!=NULL){?>
