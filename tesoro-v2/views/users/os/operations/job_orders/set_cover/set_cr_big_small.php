<?php
	if(isset($_POST['set_cr_big_small_save'])){
		$cub = $_SESSION['user_id'];
		$big_small_save = "UPDATE jo SET cover_updated_by='$cub',cover_updated_on=NOW()";

		if(isset($_POST['cover']) && $_POST['cover']!=NULL){
			$cover = $_POST['cover'];
			$big_small_save .= ",cover='$cover'";
			...
			$big_small_save .= ",cover=NULL";
		}

		$big_small_save .= " WHERE job_no=".$_GET['set_cr'];
...
<form action="<?php echo $_SESSION['page'];?>?set_cr=<?php echo $_GET['set_cr'];?>" method="POST">
	...
	<div style="float: right; padding: 20px 0px">
		...
		<button class="btn btn-primary" type="submit" name="set_cr_big_small_save">Save Changes</button>
	</div>
</form>