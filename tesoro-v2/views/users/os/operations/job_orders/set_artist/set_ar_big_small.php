<?php
	if(isset($_POST['set_ar_big_small_save'])){
		$aab = $_SESSION['user_id'];
		$big_small_save = "UPDATE jo SET artist_assigned_by='$aab',artist_assigned_on=NOW()";

		if(isset($_POST['artist']) && $_POST['artist']!=NULL){
			$artist = $_POST['artist'];
			$big_small_save .= ",artist='$artist'";
			...
			$big_small_save .= ",artist=NULL";
		}

		$big_small_save .= " WHERE job_no=".$_GET['set_ar'];
...
<form action="<?php echo $_SESSION['page'];?>?set_ar=<?php echo $_GET['set_ar'];?>" method="POST">
	...
	<div style="float: right; padding: 20px 0px">
		...
		<button class="btn btn-primary" type="submit" name="set_ar_big_small_save">Save Changes</button>
	</div>
</form>