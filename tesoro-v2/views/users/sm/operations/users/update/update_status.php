<?php
	if(isset($_POST['update_user'])){
		$type = $_POST['type'];
		$status = $_POST['status'];
		$update_user = "UPDATE users_list SET type='$type',status='$status' WHERE id=".$_GET['update'];
	...
	$query="SELECT * FROM users_list WHERE id=".$_GET['update'];
			...
			<div class="form-group row">
				...
				<div class="col-12">
					<select class="custom-select" name="type" id="type" required>
						...
						$type_query="SELECT * FROM users_type";
						$type_result=mysqli_query($conn,$type_query);
						if(mysqli_num_rows($type_result)>0){
							while($type=mysqli_fetch_array($type_result)){
								if($type['id']==$row['type']){?>
									<option value="<?php echo $type['id'];?>" selected><?php echo $type['type'];?></option><?php
								}else{?>
									<option value="<?php echo $type['id'];?>"><?php echo $type['type'];?></option><?php
								}
							}
						}else{?>
							Not Available<?php
					...
					<select class="custom-select" name="status" id="status" required>
						<option value="" disabled selected>Select</option><?php
							if($row['status']=="Active"){?>
								...
								<option value='Inactive' selected>Inactive</option><?php
							}?>
					</select>
				</div>
			</div>
