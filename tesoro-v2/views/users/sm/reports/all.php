<?php
	$_SESSION['page'] = "reports.php?reports=all";
?>
<title>System Reports | All</title>
<ul id="tabsJustified" class="nav nav-tabs">
	<li class="nav-item">
		<a href="reports.php?reports=all" class="nav-link small text-uppercase active">
			All
	...
	if(mysqli_num_rows($sr_result)>0){
		while($sr=mysqli_fetch_assoc($sr_result)){
			if($sr['id']==1){?>
				...
				<li class="nav-item">
					<a href="reports.php?reports=suggestions" class="nav-link small text-uppercase">
						<?php echo $sr['type'];?>
						<span class="float-right badge badge-pill badge-danger" style="margin-left:10px"><?php
...
<div id="tabsJustifiedContent" class="tab-content">
	<div id="all_jobs" class="tab-pane fade active show">
		<div class="table-responsive">
			...
			<tbody><?php
				$sr_query="SELECT a.id,a.message,a.sent_on,a.status,b.type,c.firstname,c.lastname FROM system_reports a LEFT JOIN system_reports_type b ON a.report_type=b.id LEFT JOIN users_list c ON a.sent_by=c.id ORDER BY a.sent_on DESC";
				$sr_result=mysqli_query($conn,$sr_query);
					if(mysqli_num_rows($sr_result)>0){
						while($sr=mysqli_fetch_assoc($sr_result)){?>  
							<tr> 
								...
								<td><?php
									...
									}else if($sr['type']=='Suggestions'){?>
										<span class="badge badge-primary text-uppercase"><?php
											echo $sr['type'];?>
										</span><?php										
									}?>
									</span>
