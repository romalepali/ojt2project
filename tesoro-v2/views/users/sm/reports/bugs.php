<?php
	$_SESSION['page'] = "reports.php?reports=bugs";
?>
<title>System Reports | Bugs</title>
<ul id="tabsJustified" class="nav nav-tabs">
	...
	if(mysqli_num_rows($sr_result)>0){
		while($sr=mysqli_fetch_assoc($sr_result)){
			if($sr['id']==1){?>
				<li class="nav-item">
					<a href="reports.php?reports=bugs" class="nav-link small text-uppercase active">
						<?php echo $sr['type'];?>
						<span class="float-right badge badge-pill badge-danger" style="margin-left:10px"><?php
...
<div id="tabsJustifiedContent" class="tab-content">
	<div id="all_jobs" class="tab-pane fade active show">
		<div class="table-responsive">
			...
			<tbody><?php
				$sr_query="SELECT a.id,a.message,a.sent_on,a.status,b.type,c.firstname,c.lastname FROM system_reports a LEFT JOIN system_reports_type b ON a.report_type=b.id LEFT JOIN users_list c ON a.sent_by=c.id WHERE b.id=1 ORDER BY a.sent_on DESC";
				$sr_result=mysqli_query($conn,$sr_query);
					if(mysqli_num_rows($sr_result)>0){
						while($sr=mysqli_fetch_assoc($sr_result)){?>  
							<tr> 
